...
bool Board::swapBlocks() {
	...
	if (swappable(cursor.row, cursor.column) && swappable(cursor.row, cursor.column+ 1))
	{
		...
		*/
		...
		tiles[cursor.row][cursor.column].block.stateExtra = BlockExtraState::swappingLeft;//goes towards right, but already is
		tiles[cursor.row][cursor.column + 1].block.stateExtra = BlockExtraState::swappingRight;//goes towards left, but already is
...
bool Board::_checkMatch(int row, int column, int row2, int column2)
{
	...
	if (row2 >= 0 && row2 < BOARD_HEIGHT && column2 >= 0 && column2 < BOARD_WIDTH)
	{
		...
		if (tiles[row][column].isBlock() && tiles[row2][column2].isBlock())
		{
			...
			if (tiles[row][column].block.state == BlockState::normal && 
				tiles[row2][column2].block.state == BlockState::normal)//maybe add landed as well
			{
					...
					if (i >= 1)
					{
						if (tiles[i][j].fallingCounter <= 0) {
							...
							if (tiles[i - 1][j].block.state == BlockState::cleared && tiles[i][j].block.stateExtra != BlockExtraState::fromClear)//skill chain mechanics
							{
								...
								/*TODO:: Make Sure this skill chain mechanic will work fine*/
								//tiles[i-1][j] = tiles[i][j];
								//tiles[i][j].type = BlockType::air;
							}
						}
...
There is 1 frame on the moment the tiles are cleared, that the chain ends prematurely
there is also one frame upon the landing of the blocks where the chain ends as well
*********/
